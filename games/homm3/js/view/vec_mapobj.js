...
(function (root) {
  ...
  function obj(name, build, o) {
    ...
    V.def(name, { w: fr.w, h: Math.min(fr.h, anchor[1] + 26), anchor, parts: [{ kind: 'torso', pivot: anchor.slice(), shapes }] });
  }
  ...
  /* =================================================================== шахты
     Скальный холм, штольня с крепью и фонарём, рельсы, вагонетка с добычей,
     куча ресурса у входа. Флаг владельца рисует карта сама (drawFlag) — здесь его нет. */
  function mine(rid) {
    const out = [];
    // холм: общий силуэт, освещённая левая грань, теневая правая, осыпь
    out.push(sh([P(14, 288, 1), [22, 238], [44, 186], [74, 140], [112, 110], [162, 96], [214, 104], [256, 130], [288, 172], [308, 226], P(316, 288, 1)], '#8c857a', 'horn', { gloss: 0.12, belly: 0.35 }));
    out.push(sh([P(30, 260, 1), [48, 196], [80, 146], [118, 114], P(160, 100, 1), [138, 132], [112, 170], [84, 230], P(70, 276, 1)], '#a8a194', 'horn', { gloss: 0.1, line: 0.4, ao: 0.4 }));
    out.push(sh([P(214, 106, 1), [252, 134], [286, 176], [304, 226], P(310, 282, 1), [276, 262], [252, 206], [232, 150]], '#6c665e', 'horn', { gloss: 0.05, line: 0.4, ao: 0.5 }));
    out.push(sh([[120, 124], [164, 112], [200, 120], [176, 132], [140, 134]], '#b8b1a4', 'horn', { line: 0.3, ao: 0.3 }));
    out.push(boulder(292, 272, 20, 12, '#7a746a'), boulder(36, 276, 16, 10, '#8a8478'));
    // штольня: тёмный проём, стойки и верхняк крепи, раскос, фонарь
    ...
    out.push(sh(tube([[62, 150, 3], [60, 136, 3]]), '#4a3020', 'wood', { line: 0.4 }));
    ...
    // рельсы: подсыпка, шпалы, две нити
    const sleepers = []; for (let x = 128; x < 302; x += 16) sleepers.push(ln([[x, 272], [x - 3, 285]], 4, 0.9, { c: '#5a3a1c' }));
    out.push(sh([P(110, 288, 1), [200, 287], P(306, 284, 1), P(302, 270, 1), [200, 271], P(120, 272, 1)], '#6e6254', 'cloth', { line: 0.4, ao: 0.4,
      lines: [...sleepers, ln([[112, 275], [304, 273]], 2, 0.9, { c: '#c8ccd4' }), ln([[110, 283], [306, 281]], 2.2, 0.9, { c: '#c8ccd4' })] }));
    // вагонетка
    out.push(...place(RES[rid](), 244, 220, 0.85));
    out.push(sh([P(202, 208, 1), P(286, 208, 1), P(278, 256, 1), P(210, 256, 1)], '#8a5a32', 'wood', { flow: 0,
      lines: [ln([[206, 224], [283, 224]], 1.2, 0.5), ln([[208, 240], [281, 240]], 1.2, 0.5), ln([[204, 210], [284, 210]], 2.4, 0.6, { light: true }), ln([[205, 212], [212, 254]], 3, 0.8, { c: IRON }), ln([[283, 212], [276, 254]], 3, 0.8, { c: IRON })] }));
    out.push(...wheel(224, 264, 12, '#3e3830', { n: 6 }), ...wheel(266, 264, 12, '#3e3830', { n: 6, a: 0.4 }));
    // куча добытого у входа
  ...
  machine('ballista', f => {
    const a = f.anchor;
    const dir = [0.984, -0.179], nrm = [0.179, 0.984], tip = [176, 104];
    return [
      { kind: 'torso', pivot: a.slice(), shapes: [
        el(94, 194, 70, 6, '#2a2418', 'flat', { line: 0 }),
        ...wheel(58, 176, 21, '#5a3a22'),
        sh(tube([[96, 154, 10], [98, 124, 9]], { flat0: true }), '#6a4424', 'wood', { flow: -Math.PI / 2 }),
        sh(tube([[28, 148, 14], [166, 112, 11]], { flat0: true, flat1: true }), '#8a5a32', 'wood', { flow: -0.25, lines: [ln([[40, 142], [160, 112]], 1.2, 0.5, { light: true })] }),
        sh([P(34, 150, 1), P(152, 150, 1), P(146, 172, 1), P(40, 172, 1)], '#7a4e2a', 'wood', { flow: 0, lines: [ln([[36, 160], [150, 160]], 1.2, 0.5), ln([[36, 151], [36, 171]], 3, 0.8, { c: IRON }), ln([[150, 151], [146, 171]], 3, 0.8, { c: IRON })] }),
        el(93, 161, 8, 8, TEAM, 'leather', { gloss: 0.5, line: 0.7, lc: '#2a1a10' }),
        ...wheel(128, 176, 21, '#5a3a22', { a: 0.3 }),
      ] },
      { kind: 'prop', pivot: [92, 118], shapes: [
        sh(tube([[142, 64, 1.6], [92, 118, 1.8], [142, 164, 1.6]]), ROPE, 'flat', { line: 0.3, lc: '#6a5a40' }),
        sh(tube([[88, 120, 4], [tip[0], tip[1], 4]], { flat1: true }), '#a8743e', 'wood', { line: 0.6 }),
        sh([P(tip[0] + nrm[0] * 6, tip[1] + nrm[1] * 6, 1), P(tip[0] + dir[0] * 18, tip[1] + dir[1] * 18, 1), P(tip[0] - nrm[0] * 6, tip[1] - nrm[1] * 6, 1)], '#c8d0d8', 'steel', { line: 0.6 }),
        sh(tube([[140, 62, 5], [150, 84, 8], [154, 114, 10], [150, 144, 8], [140, 166, 5]]), '#6a4424', 'horn', { gloss: 0.3, line: 0.8, lines: [ln([[152, 100], [154, 128]], 3, 0.8, { c: IRON })] }),
        el(88, 120, 4, 4, IRON, 'steel', { line: 0.4 }),
      ] },
    ];
  });
  ...
  machine('ammo_cart', f => {
    const arrows = []; for (let i = 0; i < 7; i++) { const x = 40 + i * 6, a = (i - 3) * 0.06; arrows.push(sh(tube([[x, 96, 2], [x + Math.sin(a) * 50, 96 - Math.cos(a) * 50, 2]]), '#c8a070', 'wood', { line: 0.3 }), sh([P(x + Math.sin(a) * 46 - 3, 58 - Math.cos(a) * 8, 1), P(x + Math.sin(a) * 50, 42, 1), P(x + Math.sin(a) * 46 + 3, 58 - Math.cos(a) * 8, 1)], '#f0ece0', 'feather', { line: 0.4 })); }
    return [{ kind: 'torso', pivot: f.anchor.slice(), shapes: [
      el(92, 160, 74, 6, '#2a2418', 'flat', { line: 0 }),
      ...flag(28, 94, 40, 28, 14, TEAM, { tri: true, pw: 3 }),
      ...arrows,
      ...barrel(118, 98, 34, 40, '#8a5a32'),
      el(146, 92, 9, 9, '#3a3a42', 'steel', { line: 0.6 }), el(134, 94, 8, 8, '#3a3a42', 'steel', { line: 0.6 }),
      sh(tube([[150, 130, 6], [182, 142, 4]]), '#6a4424', 'wood', { line: 0.6 }),
      sh([P(22, 90, 1), P(160, 90, 1), P(152, 142, 1), P(30, 142, 1)], '#8a5a32', 'wood', { flow: 0, lines: [ln([[24, 107], [158, 107]], 1.2, 0.5), ln([[26, 124], [156, 124]], 1.2, 0.5), ln([[24, 92], [158, 92]], 2.4, 0.6, { light: true }), ln([[24, 92], [30, 140]], 3.2, 0.85, { c: IRON }), ln([[158, 92], [152, 140]], 3.2, 0.85, { c: IRON }), ln([[92, 92], [92, 140]], 3, 0.8, { c: IRON })] }),
      el(92, 116, 7, 7, TEAM, 'leather', { gloss: 0.5, line: 0.7, lc: '#2a1a10' }),
      ...wheel(54, 144, 19, '#5a3a22'), ...wheel(130, 144, 19, '#5a3a22', { a: 0.3 }),
    ] }];
  });
