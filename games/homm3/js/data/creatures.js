/* ============================================================================
   data/creatures.js — 182 существа тринадцати фракций (статы SoD, см. docs/01-research.md §4).
...
(function (root) {
  ...
  const LIST = [].concat(
    ...
    /* ---------- Улей: рой ----------
       Своя фракция, не из оригинала. Идея: воюешь числом, а не качеством.
       Прирост примерно вдвое выше обычного для тира, здоровье и урон — около
       двух третей, цена — чуть больше половины. Матка медленная (тир 7 у всех
       летает и бегает, у Улья — ползёт), зато поднимает павших своих в личинок:
       рой не кончается, пока жива матка. */
    mk('hive', [
      ['larva', 'Личинка', 1, 3, 2, 1, 2, 5, 3, 18, 35, []],
      ['blood_larva', 'Кровавая личинка', 1, 4, 3, 1, 3, 7, 4, 18, 45, ['regenerate']],
      ['worker', 'Рабочий', 2, 5, 4, 2, 3, 10, 5, 11, 75, []],
      ['builder', 'Строитель', 2, 6, 6, 2, 4, 12, 5, 11, 100, ['moraleBonus']],
      ['spitter', 'Плевун', 3, 7, 5, 3, 5, 14, 5, 9, 130, ['shooter:6']],
      ['acid_spitter', 'Кислотный плевун', 3, 8, 6, 3, 6, 16, 5, 9, 165, ['shooter:8', 'ignoreDefense:30']],
      ['wasp_warrior', 'Оса-воин', 4, 10, 7, 5, 8, 22, 8, 7, 210, [FLY]],
      ['wasp_reaver', 'Оса-опустошитель', 4, 11, 8, 5, 9, 25, 9, 7, 250, [FLY, 'onHit:poison:50']],
      ['mantis', 'Богомол', 5, 13, 9, 8, 13, 32, 7, 5, 330, ['strikeAndReturn']],
      ['mantis_reaper', 'Богомол-жнец', 5, 14, 10, 8, 14, 36, 8, 5, 395, ['strikeAndReturn', 'noRetaliation']],
      ['ram_beetle', 'Жук-таран', 6, 14, 16, 12, 18, 58, 5, 3, 620, ['large']],
      ['fortress_beetle', 'Жук-крепость', 6, 15, 19, 12, 20, 65, 5, 3, 720, ['large', 'spellDamageReduce:50']],
      ['queen', 'Матка', 7, 16, 16, 20, 32, 110, 4, 2, 1550, ['large', 'raise:larva'], { gems: 1 }],
      ['ancient_queen', 'Древняя матка', 7, 18, 18, 22, 36, 130, 5, 2, 1870, ['large', 'raise:larva', 'regenerate'], { gems: 1 }],
    ]),

    /* ---------- Бастион: звери и следопыты ----------
       Своя фракция. Лесной народ, приручающий зверьё: пять зверей — быстрые и дешёвые,
       два человека-стрелка — редкие и сильные. Магии почти нет (гильдия 3), зато
       вепрь разгоняется, росомаха отвечает дважды, а великий олень поднимает мораль. */
    mk('bastion', [
      ['forest_cat', 'Лесной кот', 1, 4, 3, 1, 3, 7, 7, 14, 55, []],
      ['lynx', 'Рысь', 1, 5, 4, 2, 3, 9, 8, 14, 70, ['noRetaliation']],
      ['tracker', 'Следопыт', 2, 6, 3, 2, 3, 11, 5, 8, 110, ['shooter:12']],
      ['forester', 'Лесник', 2, 7, 4, 2, 4, 13, 6, 8, 140, ['shooter:16', 'noRangePenalty']],
      ['boar', 'Вепрь', 3, 9, 6, 3, 6, 24, 7, 7, 190, ['jousting']],
      ['tusker', 'Клыкач', 3, 10, 7, 4, 7, 28, 8, 7, 230, ['jousting', 'immuneJoust']],
      ['wolverine', 'Росомаха', 4, 11, 9, 6, 9, 34, 7, 4, 310, ['retaliations:2']],
      ['dire_wolverine', 'Матёрая росомаха', 4, 12, 10, 6, 10, 38, 8, 4, 370, ['retaliations:2', 'regenerate']],
      ['ranger', 'Ловчий', 5, 12, 9, 9, 13, 36, 6, 2, 560, ['shooter:10', 'noMeleePenalty']],
      ['master_ranger', 'Мастер-ловчий', 5, 13, 10, 9, 14, 40, 7, 2, 640, ['shooter:14', 'noMeleePenalty', 'noRangePenalty']],
      ['bear', 'Медведь', 6, 15, 14, 16, 24, 90, 6, 2, 900, ['large']],
      ['cave_bear', 'Пещерный медведь', 6, 16, 16, 18, 26, 105, 6, 2, 1050, ['large', 'regenerate']],
      ['great_stag', 'Великий олень', 7, 18, 17, 30, 45, 170, 11, 1, 2400, ['large', 'jousting', 'moraleBonus'], { crystal: 1 }],
      ['forest_king', 'Король леса', 7, 20, 19, 32, 50, 190, 12, 1, 2800, ['large', 'jousting', 'moraleBonus', 'resistAura'], { crystal: 1 }],
    ]),
  ...
  const ABILITY_NAMES = {
    ...
    wallShooter: 'Стреляет по стенам', resurrectOnce: 'Воскрешение (раз за бой)', raiseDemons: 'Поднимает демонов', raise: 'Поднимает павших своих', castRandomBuff: 'Благословляет союзников',
