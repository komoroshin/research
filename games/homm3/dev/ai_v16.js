/* ============================================================================
   dev/ai_v16.js — СНИМОК ИИ версии 1.6, только для замеров.
   Не подключается к игре: экспортируется как H3.AIOld и используется
   в dev/aiwar.js, чтобы новый ИИ играл против старого на равных картах.
   Ниже — исходный файл v1.6 без изменений, кроме имени экспорта.
...
(function (root) {
  ...
  async function runBattle(state, hero, target, hooks, smart, think) {
    ...
    // счётчик для замеров (не влияет на решения) — такой же, как в новом ИИ
    if (sum) {
      const st = state._aiStats || (state._aiStats = {});
      const me = st[hero.owner] || (st[hero.owner] = { fights: 0, lost: 0, lostValue: 0 });
      me.fights++;
      if (!sum.attWon) me.lost++;
      me.lostValue += sum.res.sides[0].lostValue || 0;
    }
  ...
  H3.AIOld = { playTurn, buildInTown, recruitInTown, chooseTarget, autoLevelUp, buySpellbook, buyMachine, heroPower };
  if (typeof module !== 'undefined' && module.exports) module.exports = H3.AIOld;
